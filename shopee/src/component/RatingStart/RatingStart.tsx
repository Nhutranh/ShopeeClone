...
import { QueryConfig } from 'src/pages/Component/ProductList/ProductList'
...
export default function RatingStart({ queryConfig }: Props) {
  ...
  const hanldeFilterStart = (ratingFilter: number) => {
    navigate({
      pathname: path.home,
        ...
        .map((_, index) => (
          <li key={index} className='py-1 pl-2'>
            <div
              ...
              onClick={() => hanldeFilterStart(5 - index)}
              aria-hidden='true'
            >
              {Array(5)
                .fill(0)
                .map((_, indexStart) => {
                  if (indexStart < 5 - index) {
                    return (
                      <svg key={indexStart} viewBox='0 0 9.5 8' className='h-4 w-4 mr-1'>
                        <defs>
                    ...
                    return (
                      <svg key={indexStart} viewBox='0 0 30 30' className='h-4 w-4 mr-1'>
                        <defs>
